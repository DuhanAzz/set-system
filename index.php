...
<body class="bg-slate-50 text-slate-900 min-h-screen flex flex-col overflow-x-hidden antialiased">
    ...
    <nav id="navbar" class="fixed w-full z-50 top-0 start-0 transparent px-6 md:px-10">
        <div class="max-w-screen-2xl flex items-center justify-between mx-auto w-full">
            <a href="index.php"><img src="set-swim-system/public/img/logo.png" class="h-16 md:h-20 w-auto object-contain transition-all duration-300" id="nav-logo"></a>
            ...
            <div class="flex items-center gap-12">
                ...
                <div class="flex items-center lg:border-l lg:border-white/20 lg:pl-10">
                    <a href="login.php" class="bg-[#f25822] hover:bg-orange-600 text-white px-8 py-3 rounded font-black text-xs uppercase tracking-widest shadow-xl transition transform hover:-translate-y-1">Login Admin</a>
                </div>
            </div>
        </div>
    </nav>
            ...
            <?php if (!empty($sliders)): ?>
                <?php foreach ($sliders as $index => $slide): ?>
                    <div class="hero-img absolute inset-0 w-full h-full transition-opacity duration-1500 <?= $index === 0 ? 'opacity-100' : 'opacity-0' ?>">
                        <img src="<?= htmlspecialchars(ltrim($slide['image_path'], '/')) ?>" alt="Hero Slider" class="object-cover w-full h-full transform scale-105">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-12 lg:gap-16">
                ...
                <div class="group relative bg-white overflow-hidden shadow-xl">
                    <div class="relative aspect-video w-full overflow-hidden bg-slate-200">
                        ...
                        <img src="<?= htmlspecialchars($swimDemo) ?>" alt="Swim App" class="object-cover object-top w-full h-full group-hover:scale-105 transition-transform duration-700">
                    </div>
                    <div class="p-8 text-center bg-white border-t-4 border-[#1e293b]">
                        ...
                        <a href="set-swim-system/public/" class="inline-block bg-[#1e293b] text-white font-bold uppercase tracking-widest text-xs px-8 py-3 hover:bg-[#f25822] transition-colors">
                            Buka Sistem
                        </a>
                    </div>
                </div>
                ...
                <div class="group relative bg-white overflow-hidden shadow-xl">
                    <div class="relative aspect-video w-full overflow-hidden bg-slate-200">
                        ...
                        <img src="<?= htmlspecialchars($rollDemo) ?>" alt="Roll App" class="object-cover object-top w-full h-full group-hover:scale-105 transition-transform duration-700">
                    </div>
                    <div class="p-8 text-center bg-white border-t-4 border-[#1e293b]">
            ...
            <div class="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
                <?php if (!empty($events)): ?>
                    <?php foreach ($events as $index => $e): ?>
                        <div class="group bg-slate-50 border border-slate-200 shadow-sm p-8 hover:shadow-xl transition-all flex flex-col cursor-pointer relative" onclick="window.location.href='set-swim-system/public/events.php?id=<?= $e['id'] ?>'">
                            <div class="absolute top-6 right-6 w-16 h-16 opacity-20 grayscale group-hover:grayscale-0 group-hover:opacity-100 transition-all duration-500">
                                ...
                                ?>
                                    <img src="<?= htmlspecialchars($swimLogoImg) ?>" alt="Swim Event" class="w-full h-full object-contain">
                                <?php else: ?>
                                    ...
                                    <img src="<?= htmlspecialchars($rollLogoImg) ?>" alt="Roll Event" class="w-full h-full object-contain">
                                <?php endif; ?>
                            </div>
                ...
                <div class="bg-white p-10 shadow-[0_5px_15px_rgba(0,0,0,0.05)] text-center group border-t-2 border-transparent hover:border-[#f25822] transition-all">
                    <div class="relative w-16 h-16 mx-auto mb-6">
                        ...
                        <?php if (!empty($settings['feature_1_icon'])): ?>
                            <img src="<?= htmlspecialchars($settings['feature_1_icon']) ?>" class="w-8 h-8 relative z-10 m-auto mt-4 object-contain group-hover:scale-110 transition-transform">
                        <?php else: ?>
                ...
                <div class="bg-white p-10 shadow-[0_5px_15px_rgba(0,0,0,0.05)] text-center group border-t-2 border-transparent hover:border-[#f25822] transition-all">
                    <div class="relative w-16 h-16 mx-auto mb-6">
                        ...
                        <?php if (!empty($settings['feature_2_icon'])): ?>
                            <img src="<?= htmlspecialchars($settings['feature_2_icon']) ?>" class="w-10 h-10 relative z-10 m-auto mt-3 object-contain group-hover:scale-110 transition-transform">
                        <?php else: ?>
                ...
                <div class="bg-white p-10 shadow-[0_5px_15px_rgba(0,0,0,0.05)] text-center group border-t-2 border-transparent hover:border-[#f25822] transition-all">
                    <div class="relative w-16 h-16 mx-auto mb-6">
                        ...
                        <?php if (!empty($settings['feature_3_icon'])): ?>
                            <img src="<?= htmlspecialchars($settings['feature_3_icon']) ?>" class="w-10 h-10 relative z-10 m-auto mt-3 object-contain group-hover:scale-110 transition-transform">
                        <?php else: ?>
                ...
                <div class="bg-white p-10 shadow-[0_5px_15px_rgba(0,0,0,0.05)] text-center group border-t-2 border-transparent hover:border-[#f25822] transition-all">
                    <div class="relative w-16 h-16 mx-auto mb-6">
                        ...
                        <?php if (!empty($settings['feature_4_icon'])): ?>
                            <img src="<?= htmlspecialchars($settings['feature_4_icon']) ?>" class="w-10 h-10 relative z-10 m-auto mt-3 object-contain group-hover:scale-110 transition-transform">
                        <?php else: ?>
    ...
    <!-- PROMO CTA SECTION -->
    <section class="relative py-40 bg-[url('https://images.unsplash.com/photo-1541252876174-8b6a3863456f?auto=format&fit=crop&q=80')] bg-cover bg-fixed bg-center">
    ...
    <footer class="bg-slate-950 text-white pt-20 pb-10 mt-auto relative z-10">
        <div class="max-w-screen-xl mx-auto px-6 grid grid-cols-1 md:grid-cols-3 gap-12 mb-16 text-center md:text-left">
            <div>
                <img src="set-swim-system/public/img/logo.png" class="h-16 mx-auto md:mx-0 mb-6 grayscale brightness-200 opacity-50 hover:opacity-100 hover:grayscale-0 transition-all">
