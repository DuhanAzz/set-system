...
<body class="bg-slate-50 min-h-screen flex items-center justify-center py-10 px-4">
    ...
    <div class="w-full max-w-md bg-white p-8 rounded-2xl shadow-xl border border-slate-200 relative">
        <a href="<?= getenv('APP_URL') ?>/swim/login" class="absolute top-4 left-4 text-slate-400 hover:text-blue-600 transition duration-300 font-bold text-xs uppercase tracking-widest">&larr; Kembali</a>
        
        <div class="text-center mb-8 mt-4">
            <h1 class="text-3xl font-black text-slate-900 uppercase tracking-tight">Buat Akun</h1>
            <p class="text-slate-500 text-sm mt-1 font-medium">Daftarkan klub renang Anda sekarang.</p>
        </div>
        ...
        <form action="<?= getenv('APP_URL') ?>/swim/register/process" method="POST" class="space-y-5">
            <div>
                <label class="block text-xs font-bold text-slate-700 uppercase mb-1.5 tracking-wide">Nama Lengkap (Admin Klub)</label>
                <input type="text" name="nama" class="w-full px-4 py-3 border border-slate-200 bg-slate-50 focus:bg-white rounded-xl focus:ring-2 focus:ring-blue-600 outline-none transition font-semibold" required>
            </div>
            <div>
                <label class="block text-xs font-bold text-slate-700 uppercase mb-1.5 tracking-wide">Nama Klub Renang</label>
                <input type="text" name="nama_klub" class="w-full px-4 py-3 border border-slate-200 bg-slate-50 focus:bg-white rounded-xl focus:ring-2 focus:ring-blue-600 outline-none transition font-semibold" required placeholder="Contoh: Tirta Jaya SC">
            </div>
            <div>
                <label class="block text-xs font-bold text-slate-700 uppercase mb-1.5 tracking-wide">No. WhatsApp</label>
                <input type="text" name="phone" class="w-full px-4 py-3 border border-slate-200 bg-slate-50 focus:bg-white rounded-xl focus:ring-2 focus:ring-blue-600 outline-none transition font-semibold" required placeholder="08...">
            </div>
            <div>
                <label class="block text-xs font-bold text-slate-700 uppercase mb-1.5 tracking-wide">Email</label>
                <input type="email" name="email" class="w-full px-4 py-3 border border-slate-200 bg-slate-50 focus:bg-white rounded-xl focus:ring-2 focus:ring-blue-600 outline-none transition font-semibold" required>
            </div>
            <div>
                <label class="block text-xs font-bold text-slate-700 uppercase mb-1.5 tracking-wide">Password</label>
                <input type="password" name="password" class="w-full px-4 py-3 border border-slate-200 bg-slate-50 focus:bg-white rounded-xl focus:ring-2 focus:ring-blue-600 outline-none transition font-semibold" required>
            </div>
            <button type="submit" class="w-full bg-blue-600 text-white font-black py-4 rounded-xl hover:bg-blue-700 transition shadow-lg mt-4 uppercase text-sm tracking-wide transform hover:-translate-y-0.5">Daftar Sekarang</button>
        </form>

        <p class="mt-8 text-center text-sm text-slate-500 font-medium">
            Sudah punya akun? <a href="<?= getenv('APP_URL') ?>/swim/login" class="text-blue-600 font-bold hover:underline">Login di sini</a>
        </p>
    </div>
