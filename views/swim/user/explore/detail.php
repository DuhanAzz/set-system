<div class="bg-white rounded-2xl shadow-sm border border-slate-200 p-8 mb-8 relative overflow-hidden">
    ...
    <div class="relative z-10 flex flex-col md:flex-row gap-6 items-start">
        <?php 
            $logoPath = '';
            if(!empty($eventInfo['logo_left'])) {
                if(filter_var($eventInfo['logo_left'], FILTER_VALIDATE_URL)) {
                    $logoPath = $eventInfo['logo_left'];
                } else {
                    $clean = ltrim(preg_replace('/^(\.\.\/)+/', '', $eventInfo['logo_left']), '/');
                    $logoPath = getenv('APP_URL') . '/public/' . $clean;
                }
            }
        ?>
        <?php if(!empty($logoPath)): ?>
            <img src="<?= htmlspecialchars($logoPath) ?>" onerror="this.onerror=null; this.src=''; this.className='hidden'; this.nextElementSibling.classList.remove('hidden');" alt="Logo Event" class="w-32 h-32 object-contain bg-white p-2 rounded-2xl border border-slate-100 shadow-sm shrink-0">
            <div class="hidden w-32 h-32 bg-slate-800 rounded-2xl flex items-center justify-center text-5xl shadow-sm border border-slate-200 shrink-0">🏊</div>
        <?php else: ?>
