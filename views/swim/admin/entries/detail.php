    ...
    <div class="space-y-6 h-fit sticky top-24">
        ...
        <div class="bg-white rounded-3xl p-6 border border-slate-200 shadow-sm">
            ...
            <?php if(!empty($payData['file_path'])): ?>
                <a href="<?= getenv('APP_URL') ?>/public/uploads/payments/<?= htmlspecialchars($payData['file_path']) ?>" target="_blank" class="block group relative rounded-xl overflow-hidden border border-slate-200 bg-slate-100 aspect-video flex items-center justify-center cursor-pointer shadow-sm mb-4">
                    ...
                    <?php if(in_array(strtolower($ext), ['jpg','jpeg','png'])): ?>
                        <img src="<?= getenv('APP_URL') ?>/public/uploads/payments/<?= htmlspecialchars($payData['file_path']) ?>" class="object-contain w-full h-full">
                    <?php else: ?>
...
function openModal(action) {
    const msg = action === 'approve' ? 'Terima & Kunci Data?' : 'Tolak & Minta Revisi?';
