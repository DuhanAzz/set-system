...
<div class="max-w-[210mm] mx-auto mb-6 space-y-4 no-print" id="main-content">
    <div class="bg-white p-4 rounded-xl shadow border border-slate-200 flex flex-col md:flex-row justify-between items-center gap-4">
        ...
        <div class="flex gap-2 bg-slate-100 p-1 rounded-lg">
            <a href="<?= getenv('APP_URL') ?>/swim/medal_tally?team_source=<?= htmlspecialchars($team_source) ?>" class="bg-white shadow text-blue-700 px-4 py-1.5 rounded text-xs font-bold uppercase transition">
                🏆 Juara Umum (Tim)
            </a>
            <a href="<?= getenv('APP_URL') ?>/swim/medal_tally/best_swimmer?team_source=<?= htmlspecialchars($team_source) ?>" class="text-gray-500 hover:text-gray-700 px-4 py-1.5 rounded text-xs font-bold uppercase transition">
                🏊‍♂️ Perenang Terbaik
            </a>
        </div>
    </div>
    ...
    <div class="bg-white p-4 rounded-xl shadow border border-blue-100">
        <form method="GET" class="space-y-4" action="<?= getenv('APP_URL') ?>/swim/medal_tally">
                        ...
                        <div class="block-tabel">
                            <div class="event-header">
                                <div class="eh-left-group">
                                    ...
                                    <div class="eh-date"><?= htmlspecialchars($raceInfo['event_date_start'] ?? '') ?></div>
                                </div>
