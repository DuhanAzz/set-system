...
if (isset($raceInfo['is_relay']) && $raceInfo['is_relay'] == 1) {
    $sql = "SELECT es.heat_prelim as heat_no, es.lane_prelim as lane_no, es.time_prelim as entry_time,
            '' as team_name, c.nama_klub as club_name, 
        ...
        <div class="content-body">
            ...
            <?php else: ?>
                ...
                <div class="event-records-container" style="border:none; padding:0; margin-bottom:10px;">
                    <?php 
                    if($pc['show_records']): 
                        $stmtRec = $pdo->prepare("SELECT record_type, holder_name, record_time, location, record_year FROM swim_master_records WHERE distance = ? AND stroke = ? AND jenis_kelamin = ? AND (age_group = ? OR record_type = 'rekornas') ORDER BY record_type ASC, id ASC");
