...
<div class="p-6 max-w-5xl mx-auto">
    <div class="flex justify-between items-center mb-8">
        <div>
            <h2 class="text-3xl font-black text-slate-800 uppercase tracking-tighter">System Health</h2>
            <p class="text-slate-500 font-bold text-sm mt-1">Status Kesehatan dan Optimasi Database.</p>
        </div>
    </div>
    ...
    <div class="max-w-xl">
        <div class="bg-amber-50 border border-amber-200 rounded-3xl p-8 relative overflow-hidden shadow-sm group">
            ...
            <div class="mb-8 p-4 bg-white/60 rounded-xl border border-amber-100 relative z-10 flex flex-col gap-3">
                <div>
                ...
                <div class="grid grid-cols-2 gap-4 mt-2 border-t border-amber-200/50 pt-4">
                    <div>
                        <span class="block text-xs uppercase font-black text-amber-500 mb-1">Total Tabel DB</span>
                        <span class="text-xl font-black text-amber-700"><?= number_format($stats['total_tables']) ?></span>
                    </div>
                    <div>
                        <span class="block text-xs uppercase font-black text-amber-500 mb-1">Total Storage</span>
                        <span class="text-xl font-black text-amber-700"><?= number_format($stats['total_size_mb'], 2) ?> <span class="text-sm">MB</span></span>
                    </div>
                </div>
            </div>

            <form action="<?= getenv('APP_URL') ?>/roll/maintenance/system_health" method="POST" class="relative z-10" onsubmit="return confirm('Proses optimasi akan mengunci tabel selama beberapa saat. Yakin lanjutkan?');">
