        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
            <?php foreach($upcoming_preview as $e): 
                ...
                if (!empty($e['poster_image'])) {
                    $imgSrc = (strpos($e['poster_image'], 'http') === 0) ? $e['poster_image'] : rtrim(getenv('APP_URL'), '/') . '/public/' . ltrim($e['poster_image'], '/');
                } elseif (!empty($e['logo_left'])) {
                    $imgSrc = (strpos($e['logo_left'], 'http') === 0) ? $e['logo_left'] : rtrim(getenv('APP_URL'), '/') . '/public/' . ltrim($e['logo_left'], '/');
                }
            ?>
