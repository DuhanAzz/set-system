...
<head>
    ...
    <title>Login - SET System Roll</title>
...
<body class="bg-white h-screen w-full flex overflow-hidden">
    ...
    <div class="w-full lg:w-1/2 flex flex-col justify-center items-center p-6 bg-white overflow-y-auto relative">
        ...
        <a href="<?= getenv('APP_URL') ?>/roll" class="absolute top-6 left-6 md:top-10 md:left-10 flex items-center gap-2 text-slate-400 hover:text-orange-600 transition duration-300 font-bold text-xs uppercase tracking-widest group">
        ...
        <div class="w-full max-w-md mt-10 lg:mt-0">
            ...
            <?php if(isset($_SESSION['flash_message'])): ?>
                <div class="bg-red-50 border-l-4 border-red-500 text-red-700 p-4 rounded-lg mb-6 text-sm flex items-center gap-3 animate-pulse">
                    <span class="font-bold"><?= htmlspecialchars($_SESSION['flash_message']) ?></span>
                </div>
                <?php unset($_SESSION['flash_message']); unset($_SESSION['flash_type']); ?>
            <?php endif; ?>

            <form action="<?= getenv('APP_URL') ?>/roll/login/submit" method="POST" class="space-y-6">
                <div>
                    <label class="block text-slate-700 font-bold mb-2 text-xs uppercase tracking-wide">Email atau Username</label>
                    <input type="text" name="username" class="w-full pl-4 pr-4 py-3.5 rounded-xl border border-slate-200 bg-slate-50 focus:bg-white focus:ring-2 focus:ring-orange-500 outline-none transition font-semibold text-slate-800" placeholder="Masukkan akun Anda" required>
                </div>
                <div>
                    <div class="flex justify-between items-center mb-2">
                        <label class="text-slate-700 font-bold text-xs uppercase tracking-wide">Password</label>
                    </div>
                    <input type="password" id="password" name="password" class="w-full pl-4 pr-10 py-3.5 rounded-xl border border-slate-200 bg-slate-50 focus:bg-white focus:ring-2 focus:ring-orange-500 outline-none transition font-semibold text-slate-800" placeholder="••••••••" required>
                </div>
                <button type="submit" class="w-full bg-[#0F172A] hover:bg-orange-600 text-white font-black py-4 rounded-xl shadow-lg transition transform hover:-translate-y-0.5 uppercase tracking-wide text-sm">Masuk Dashboard</button>
            </form>
            
            <div class="mt-8 text-center">
                <p class="text-sm text-slate-500 font-medium">Belum punya akun? <a href="<?= getenv('APP_URL') ?>/roll/register" class="text-orange-600 font-bold hover:underline">Daftar Klub Baru</a></p>
            </div>
        </div>
    </div>
