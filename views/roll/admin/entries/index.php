                    ...
                    <tr class="group hover:bg-slate-50 transition <?= $status == 'Pending' ? 'bg-amber-50/40' : '' ?>">
                        ...
                        <td class="py-4 px-4 text-center">
                            <?php if(!empty($row['file_path'])): ?>
                                <a href="<?= getenv('APP_URL') ?>/uploads/payments/<?= htmlspecialchars($row['file_path']) ?>" target="_blank" class="inline-flex items-center gap-1 px-3 py-1 rounded-lg bg-blue-50 text-blue-600 border border-blue-200 hover:bg-blue-100 text-[9px] font-bold uppercase transition">
                                    👁️ Lihat Bukti
                                </a>
                            <?php else: ?>
                        ...
                        <td class="py-4 px-6 text-right">
                            <div class="flex items-center justify-end gap-2">
                                ...
                                <a href="<?= getenv('APP_URL') ?>/roll/admin/entries/detail?id=<?= $row['club_id'] ?>&event_id=<?= $targetEventId ?>" 
                                ...
                                <?php if($status == "Paid"): ?>
                                    <form method="POST" action="<?= getenv("APP_URL") ?>/roll/admin/entries" class="inline" onsubmit="return confirm('Batal Verifikasi Lunas? Status akan kembali Pending.');">
                                ...
                                <?php if($status == 'Pending'): ?>
                                    <form method="POST" action="<?= getenv('APP_URL') ?>/roll/admin/entries" class="inline" onsubmit="return confirm('Tolak Pembayaran ini?');">
                                    ...
                                    <form method="POST" action="<?= getenv('APP_URL') ?>/roll/admin/entries" class="inline" onsubmit="return confirm('Verifikasi LUNAS?');">
