...
if (isset($_GET['action']) && isset($_GET['uid']) && isset($_GET['status'])) {
    ...
    if (in_array($newStatus, ['active', 'pending', 'suspended'])) {
        if ($uid == $_SESSION['user_id']) {
            ...
            try {
                $pdo->prepare("UPDATE swim_users SET account_status = ? WHERE id = ?")->execute([$newStatus, $uid]);
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['save_user'])) {
    try {
        ...
        if ($userId) {
            ...
            // A. Update swim_users (SUDAH TERMASUK PHONE)
            if (!empty($pass)) {
                $pdo->prepare("UPDATE swim_users SET nama_lengkap=?, email=?, phone=?, username=?, password=? WHERE id=?")
                    ->execute([$namaAkun, $email, $phone, $username, password_hash($pass, PASSWORD_DEFAULT), $userId]);
            } else {
                $pdo->prepare("UPDATE swim_users SET nama_lengkap=?, email=?, phone=?, username=? WHERE id=?")
                    ->execute([$namaAkun, $email, $phone, $username, $userId]);
            }

            // B. Update Detail
            if ($role == 'admin') {
                $check = $pdo->prepare("SELECT id FROM swim_events WHERE user_id = ?");
                $check->execute([$userId]);
                if ($check->rowCount() > 0) {
                    $pdo->prepare("UPDATE swim_events SET event_name=?, competition_system=?, event_location=?, event_city=?, event_date_start=? WHERE user_id=?")
                        ->execute([$namaEntitas, $compSystem, $location, $city, $eventDate, $userId]);
                } else {
                    $pdo->prepare("INSERT INTO swim_events (user_id, event_name, competition_system, event_location, event_city, event_date_start, event_status, event_type, lane_count, pool_type) VALUES (?, ?, ?, ?, ?, ?, 'Upcoming', 'Standard', 8, '50m')")
                        ->execute([$userId, $namaEntitas, $compSystem, $location, $city, $eventDate]);
                }
            } elseif ($role == 'user') {
                $check = $pdo->prepare("SELECT id FROM swim_clubs WHERE user_id = ?");
                $check->execute([$userId]);
                if ($check->rowCount() > 0) {
                    $pdo->prepare("UPDATE swim_clubs SET nama_klub=?, kota=? WHERE user_id=?")
                        ->execute([$namaEntitas, $kota, $userId]);
                } else {
                    $pdo->prepare("INSERT INTO swim_clubs (user_id, nama_klub, kota) VALUES (?, ?, ?)")
            ...
            // === INSERT BARU ===
            $cekMail = $pdo->prepare("SELECT id FROM swim_users WHERE email = ?");
            ...
            $pdo->prepare("INSERT INTO swim_users (nama_lengkap, email, phone, username, password, role, account_status) VALUES (?, ?, ?, ?, ?, ?, 'active')")
...
if (isset($_GET['delete'])) {
    ...
    if ($id == $_SESSION['user_id']) {
        ...
        try {
            ...
            $pdo->prepare("DELETE FROM swim_swimmers WHERE user_id = ?")->execute([$id]); 
            
            $pdo->prepare("DELETE FROM swim_events WHERE user_id = ?")->execute([$id]); 
            $pdo->prepare("DELETE FROM swim_clubs WHERE user_id = ?")->execute([$id]);
            $pdo->prepare("DELETE FROM swim_users WHERE id = ?")->execute([$id]);        
            $pdo->commit();
